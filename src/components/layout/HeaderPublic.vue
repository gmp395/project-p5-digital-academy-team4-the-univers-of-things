  ...
  <header class="fixed top-0 w-full z-50 bg-background/70 backdrop-blur-xl border-b glass-stroke py-3 transition-all duration-300">
    <nav class="flex justify-between items-center px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto">
      <div class="flex items-center gap-12">
        ...
        <div class="hidden md:flex items-center gap-8">
          <RouterLink to="/" class="font-body-sm text-body-sm text-on-surface-variant hover:text-on-surface transition-colors pb-1" active-class="text-primary font-bold border-b-2 border-primary">Home</RouterLink>
        </div>
      </div>
      ...
      <div class="flex items-center gap-6">
        <div class="hidden lg:flex items-center gap-4">
          ...
          <!-- Sin sesión -->
          <RouterLink v-if="!authStore.isAuthenticated" to="/login" class="text-on-surface-variant hover:text-on-surface transition-opacity duration-300 font-label-lg text-label-lg">Login</RouterLink>

          <!-- Admin: dropdown -->
          <div v-else-if="authStore.userRole === 'admin'" class="relative">
            ...
            <transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0 scale-95 -translate-y-1" enter-to-class="opacity-100 scale-100 translate-y-0" leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100 scale-100 translate-y-0" leave-to-class="opacity-0 scale-95 -translate-y-1">
              <div v-if="dropdownOpen" class="absolute right-0 top-12 w-56 bg-surface-container border glass-stroke rounded-2xl shadow-lg z-50 overflow-hidden">
                ...
                <div class="py-1">
                  <RouterLink to="/admin/settings" @click="dropdownOpen = false" class="flex items-center gap-3 px-4 py-2.5 text-sm text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface transition-colors">
                    <span class="material-symbols-outlined text-[18px]">settings</span>
                    Ajustes
                  </RouterLink>
                </div>
                <div class="border-t glass-stroke py-1">
                  <button @click="() => { logout(); dropdownOpen = false }" class="flex items-center gap-3 px-4 py-2.5 text-sm text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface transition-colors w-full">
          ...
          <!-- Usuario regular -->
          <template v-else>
            <RouterLink to="/user" class="flex items-center gap-2 text-on-surface-variant hover:text-on-surface transition-opacity duration-300">
            ...
            <button @click="logout" class="text-on-surface-variant hover:text-on-surface transition-opacity duration-300 font-label-lg text-label-lg">
              Cerrar sesión
            </button>
          </template>
        </div>

        <button class="lg:hidden flex items-center justify-center text-on-surface p-1" @click="isMenuOpen = !isMenuOpen" :aria-expanded="isMenuOpen" aria-label="Abrir menú de navegación">
    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-2">
      <div v-if="isMenuOpen" class="lg:hidden flex flex-col gap-3 px-margin-mobile py-4 bg-surface-dark/95 backdrop-blur-xl border-b glass-stroke">
        ...
        <template v-else>
          <RouterLink to="/user" class="text-body-md text-on-surface-variant hover:text-on-surface py-2" @click="isMenuOpen = false">{{ userName }}</RouterLink>
          <button @click="() => { logout(); isMenuOpen = false }" class="text-body-md text-on-surface-variant hover:text-on-surface py-2 text-left">Cerrar sesión</button>
        </template>
      </div>
    </transition>
